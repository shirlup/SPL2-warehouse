...
public abstract class Task<R> {
    ...
    protected int theNumberOfSubTasksFinished;
    protected Deferred<R> def ;
...
public Task(){
    ...
    protected final void whenResolved(Collection<? extends Task<?>> tasks, Runnable callback) {
        ...
        Runnable subTaskCallback =() -> {
            synchronized (this) {
                this.theNumberOfSubTasksFinished++;
                //only if all subtasks has been resolved
                if (numOfTasks == theNumberOfSubTasksFinished) {
                    addTaskToQueue();
                }
            }
        };
        ...
        for (Task<?> task : tasks) {
            task.getResult().whenResolved(subTaskCallback);
        }
    }
    ...
    /**
     * calls a method in the processor class to add the current task to the queue
     */
